...
import 'package:k3_sipp_mobile/widget/examination/input/kebisingan_noise_dose/form_kebisingan_noise_dose.dart';
import 'package:k3_sipp_mobile/widget/examination/input/kebisingan_noise_dose/input_kebisingan_noise_dose_row.dart';

class InputKebisinganNoiseDosePage extends StatefulWidget {
  final InputFormLogic logic;

  const InputKebisinganNoiseDosePage({super.key, required this.logic});

  @override
  State<InputKebisinganNoiseDosePage> createState() => _InputKebisinganNoiseDosePageState();
}

class _InputKebisinganNoiseDosePageState extends State<InputKebisinganNoiseDosePage> {

  Future<void> _addOrUpdateRow({DataNoiseDose? data}) async {
    if (!widget.logic.examination.canUpdateInput) return;

    await Navigator.of(context).push(
      MaterialPageRoute(
        builder: (context) => FormKebisinganNoiseDose(
          data: data,
          onUpdate: (input) {
            if (data != input) {
  ...
  Widget _buildData() {
    List<Widget> widgets = [];
    Map<int, DataNoiseDose> resultMap = {};
    for (DataNoiseDose result in widget.logic.examination.examinationResult) {
      resultMap[result.id!] = result;
    }
    for (DataNoiseDose input in widget.logic.examination.userInput) {
      widgets.add(InputKebisinganFrekuensiRow(input: input, result: resultMap[input.id]));
    }
