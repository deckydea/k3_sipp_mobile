...
import 'package:k3_sipp_mobile/widget/assignment/template_row.dart';
...
class TemplatePengujianPage extends StatefulWidget {
  const TemplatePengujianPage({super.key});

  @override
  State<TemplatePengujianPage> createState() => _TemplatePengujianPageState();
}

class _TemplatePengujianPageState extends State<TemplatePengujianPage> {
  DateTime _selectedDate = DateTime.now();

  Future<void> _navigateToDetail(Template template) async {
    var result = await navigatorKey.currentState?.pushNamed("/template_detail", arguments: template);
    ...
    return Container(
      ...
      child: ListView.separated(
        ...
        itemBuilder: (context, index) {
          Template template = templates.elementAt(index);
          return AssignmentTemplateRow(
            template: template,
